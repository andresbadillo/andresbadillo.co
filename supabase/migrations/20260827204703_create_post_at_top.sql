-- Mantiene el post recién creado en la posición 0 mediante una única transacción.
do $$
begin
  if exists (
    select 1
    ...
    raise exception 'posts.display_order contains duplicate values';
  end if;
end
$$;

alter table public.posts
  add constraint posts_display_order_unique
  unique (display_order)
  deferrable initially deferred;

create function public.create_post_at_top(
  p_slug text,
  p_title text,
  p_excerpt text,
  p_published_at date,
...
begin
  ...
  -- Serializa las altas para que dos publicaciones concurrentes no ocupen el 0.
